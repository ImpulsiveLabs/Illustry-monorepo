...
const forwardBundleResponse = async (response: Response) => {
  if (!response.ok) {
    const message = await response.text().catch(() => 'Unable to export visualization.');
    return NextResponse.json({ error: message || 'Unable to export visualization.' }, { status: response.status });
  }
